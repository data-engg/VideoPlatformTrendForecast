...
def read_inp_file():
    inp_file_path = get_project_path() + '/input_info.json'
    return read_json_file(inp_file_path)  # type: object
...
def prepare_meta(inp_data):
    ...
    for file_format in formats:
        output_folder_path = os.path.join(get_project_path(), 'out', meta['destination_folder_path'],
                                          str(date.today()), file_format)
...
def prepare_data(inp_data):
    ...
    for i in range(no_of_rows):
        ...
        if counter == 999:
            ...
            if json_gen is not None:
                ...
                with open(json_gen, "w") as outfile: 
    ...
    if counter > 0:
        ...
        print(list_of_dict)
        if json_gen is not None:
            #json_object = json.dumps(list_of_dict,2)
            with open(json_gen, "w") as outfile: 
                json.dump(list_of_dict, outfile)
                #outfile.write(json_object + "\n") 
